...
model = genai.GenerativeModel(require_env("GEMINI_MODEL"))
...
def validate_document_filter() -> None:
    """Verify the Azure Search filter matches at least one document before calling Gemini."""
...
def call_gemini(user_prompt: str, search_context: str = None, retries: int = 3) -> Dict[str, Any]:
    """Call Google Gemini API with retry logic on failure."""
    ...
    for attempt in range(retries):
        try:
            ...
            if search_context:
                full_prompt += f"Document Context from Azure Search:\n{search_context}\n\n"
            ...
            return {
                ...
                "tokens": len(response_content.split()),  # Approximate token count
...
def main():
    ...
    print(f"Using Google Gemini API for LLM\n")

    # Step 3: Process ALL prompts
    results = []
    total_tokens = 0

    # Retrieve search context once
    ...
    for i, (field_name, field_prompt) in enumerate(prompts.items(), 1):
        ...
        try:
            # Call Gemini API with search context
            api_response = call_gemini(field_prompt, search_context)
